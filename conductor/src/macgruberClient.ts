/**
 * Conductor-side MacGruber fire-and-forget client.
 *
 * Per FEAT.MACGRUBER.8 (D8, D9): on `fail_tactical` and `fail_strategic` verdicts,
 * Conductor pushes a structured failure payload to MacGruber's `/intake` so it can
 * investigate and remediate. The push must NOT block Conductor's own verdict flow,
 * and network errors must not propagate.
 *
 * Wiring: import `notifyMacGruber` and call it (without awaiting in a blocking way)
 * from the verdict-emission paths that exit with `fail_tactical` or `fail_strategic`.
 */
...
export async function notifyMacGruber(
  ...
  const body: MacGruberPushPayload = { ...payload, reported_by: 'conductor' };
