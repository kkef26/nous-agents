...
import type { NousDatabase } from "../_common/types.ts";
...
type AmendmentDatabase = NousDatabase & {
  nous: NousDatabase["nous"] & {
    Tables: NousDatabase["nous"]["Tables"] & {
      amendment_queue: {
        Row: AmendmentQueueRow;
        Insert: AmendmentQueueInsert;
        Update: Partial<AmendmentQueueInsert>;
      };
      dispatch_queue: {
        Row: DispatchQueueProjectRow;
        Insert: DispatchQueueProjectRow;
        Update: Partial<DispatchQueueProjectRow>;
      };
    };
  };
};

function db(): SupabaseClient<AmendmentDatabase, "nous"> {
  return getSupabaseClient() as unknown as SupabaseClient<AmendmentDatabase, "nous">;
}
