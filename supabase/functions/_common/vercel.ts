...
import type { VercelDeployment, VercelBuildLog } from "./types.ts";
...
async function getToken(): Promise<string> {
  ...
  const tok = fromConfig ?? Deno.env.get("VERCEL_TOKEN") ?? null;
  if (!tok) {
...
async function getTeamId(): Promise<string | null> {
  ...
  cachedTeam = fromConfig ?? Deno.env.get("VERCEL_TEAM_ID") ?? null;
  return cachedTeam;
}
