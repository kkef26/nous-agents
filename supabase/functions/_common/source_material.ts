// supabase/functions/_common/source_material.ts
...
export interface FeatureSourceMaterial {
  grill_decisions: SourceMaterialRow[];        // source_type = grill_decision
  project_grill_decisions: SourceMaterialRow[]; // source_type = grill_decision_project
  architecture_docs: SourceMaterialRow[];       // source_type = architecture
  grill_resolutions: SourceMaterialRow[];       // source_type = grill_resolution
  prototype_decisions: SourceMaterialRow[];     // source_type = prototype_decision
...
export async function loadFeatureSourceMaterial(
  ...
  // The view uses nulls for feature_id on project-wide rows.
  // We want: (feature_id = X) OR (project = Y)
  // PostgREST .or() syntax:
 ...
 *
 * Queries grill_decisions table directly instead of the view — faster,
 * no joins with library_artifacts.
 */
export async function countGrillDecisions(
  ...
  // deno-lint-ignore no-explicit-any
  const { count: featureCount, error: e1 } = await (sb as any)
  ...
  // deno-lint-ignore no-explicit-any
  const { count: projectCount, error: e2 } = await (sb as any)
    .from("grill_decisions")
    .select("id", { count: "exact", head: true })
    .is("feature_id", null)
    .eq("project", project);
