  ...
  if (clauseIdsToPromote.length > 0) {
    ...
    if (decomposition.generated) {
      for (const clause of decomposition.clauses) {
        const insertPayload = {
          id: clause.id, prefix: clause.prefix, parent_id: clause.parent_id,
          feature_id: clause.feature_id, sequence_order: clause.sequence_order,
          maturity_stage: "SCAFFOLD", status: "draft", clause_type: clause.clause_type,
