...
import { getConfigValue, getSupabaseClient } from "./lib/common/db.js";
import { resolveAuditTrail } from "./lib/common/audit_trail.js";
import { compareCommits } from "./lib/common/github.js";
import { getDeploymentBySha } from "./lib/common/vercel.js";
import { writeConductorStep } from "./lib/common/logging.js";
...
async function ghMerge(
  ...
  const token =
    (await getConfigValue("GITHUB_TOKEN")) ?? process.env.GITHUB_TOKEN ?? null;
  if (!token) {
...
export async function handleMerge(req: Request): Promise<Response> {
  ...
  try {
    ...
    for (const cid of clauseIds) {
      ...
      // Auto-merge contract: verify.ts emits verdict='pass' only when sentinel
      // score >= 80 (CLEAN_PASS) AND all ACs pass. Any other verdict vetoes the
      // merge — no human approval loop, no bypass path. See AGT.1.1.2.
